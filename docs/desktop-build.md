...
Electron открывает ту же автономную игру, что и `outputs/portable/FRIENDSLOP.html`.
Renderer работает без Node.js и прямого сетевого доступа. Узкий sandboxed preload
позволяет создать/остановить сервер комнаты, передать игровой запрос и скопировать приглашение.
...
загрузки, прямые сетевые запросы renderer и дополнительные разрешения. Разрешён fullscreen
главного доверенного renderer. Сетевые команды проходят через ограниченный IPC с проверкой
главного окна/фрейма и размера аргументов. Сервер SQLite/WebSocket и соединение гостя
работают в main, без доступа игрового UI к shell или файловой системе. [Рекомендации Electron](https://www.electronjs.org/docs/latest/tutorial/security).

В 0.10 доступны сетевые город, экран, казарма и переезд. «Создать игру» поднимает сервер
из приложения: LAN или временный Cloudflare-туннель. Кнопка копирует строку `WCB1:…`;
она содержит адрес сервера, ключ гостевого доступа, код комнаты и версию протокола.
Токен ведущего не копируется. Интернет-туннель объявляется готовым только после
успешного внешнего WebSocket handshake. На текущей сети TLS до Cloudflare обрывается;
для неё проверен LAN, доступны виртуальная локальная сеть или [свой сервер](server.md).

В сборки включён проверенный SHA-256 бинарник cloudflared 2026.9.1 для соответствующей
архитектуры. Он лежит вне ASAR, запускается без shell с временной конфигурацией и
останавливается вместе с сервером/окном. `network.cjs` собирается с зависимостями;
native npm-модулей и отдельной установки Node игроку не требуется. Лицензии включены.
В `.build.json` записаны хеши renderer, preload, relay, SQL и исходных модулей.

`npm run desktop:smoke` проверяет реальный preload и сервер с гостем в отдельном процессе:
вход, четыре сцены, press/release, подтверждение ввода, остановка. Это не заменяет запуск
на Windows/Intel Mac и подключение через две физические сети.
