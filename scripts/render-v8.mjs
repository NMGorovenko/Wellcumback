...
tickCity(city,1/60,new Set(t>=2&&t<7?['KeyW']:t>=9&&t<10?['KeyS']:[]));
advanceV8(s,{speed:city.speed,powertrain:city.powertrain,forward:city.vx,lateral:city.vz,throttle:city.throttle,horn:t>9&&t<9.4},1/60);graph.update(s,true,t);
...
assert(segments.every(s=>Number.isFinite(s.rms)&&s.peak<0.16),'quiet signal with headroom');assert(segments[0].rms>0.01,'audible idle');assert(segments[6].rms>segments[0].rms*1.1,'load changes sound');
assert.equal(rendered.bumps,0,'sound sample drives a real clear city road');
let full=0,bright=0;for(let i=5*48000;i<7*48000;i++){full+=data[i]*data[i];bright+=rendered.bright[i]*rendered.bright[i];}
const brightnessRatio=bright/full;assert(brightnessRatio<.012,'upper frequencies stay soft at sustained full speed');
const shifts=rendered.telemetry.filter((s,i,a)=>i>0&&s.gear>a[i-1].gear).map(s=>({time:s.time,gear:s.gear,rpm:s.rpm,minimumRpm:Math.min(...rendered.telemetry.filter(f=>f.time>=s.time&&f.time<s.time+.25).map(f=>f.rpm))}));
assert.equal(shifts.length,5);assert(shifts.every(s=>s.minimumRpm<s.rpm*.78),'audible pitch falls throughout all shifts');
