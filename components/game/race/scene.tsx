...
import {
  loadCityEnvironment,
  type CityLoadProgress,
} from '../city/environment';
    ...
    const initialize = async () => {
      ...
      const cityEnvironment =
        course.id === 'krasnoyarsk'
          ? await loadCityEnvironment(kit, abort.signal, setLoading)
          : null;
      if (abort.signal.aborted) return;
      if (!cityEnvironment) {
        setLoading({ label: 'Трасса и окружение', progress: 0.25 });
      ...
      const animate = (now: number) => {
        ...
        for (let i = 0; i < count; i++) {
          const r =
            state.racers.find((r) => r.id === ids[i]) ?? state.racers[0];
          if (!r) continue;
          const model = models.find((m) => m.id === r.id)!.model;
          for (const item of models)
            item.model.root.visible =
              state.phase !== 'lobby' || item.id === r.id;
    ...
    <div className="race-world" ref={host}>
      {loading && !failed && (
        <SceneLoading
          {...loading}
          title={
            game.current.trackId === 'nordschleife'
              ? 'Нюрбургринг'
              : 'Красноярск'
          }
        />
