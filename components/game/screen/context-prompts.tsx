...
import { screenTimingCue } from '@/lib/game/screen/timing';
  ...
  return (
    <div className="world-action-cues">
      {screenPrompts(state).map(({ worker, player, role, prompts }) => {
        const local =
          player !== null &&
          (localPlayer === undefined || localPlayer === player);
        const timing = local ? screenTimingCue(state, worker) : null;
        const timingLabel =
          timing && player !== null
            ? screenPromptInput(
                pads,
        ...
        return (
          ...
          >
            {timing ? (
              <TimingDial cue={timing} label={timingLabel} held={timingHeld} />
            ) : (
              <>
