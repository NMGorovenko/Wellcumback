...
export function CleanActionPrompts({
  ...
  return (
    <div className="world-action-cues" aria-label="Кнопки действий рядом с персонажами">
      {Array.from({ length: state.phase === "clean" ? state.actorCount : 1 }, (_, actor) => (
        <div
          key={actor}
          ref={(node) => {
        ...
        >
          <div className="context-worker-name">
            <strong>{state.phase === "clean" ? cleanCrew[actor].name : "Солдат"}</strong>
          </div>
